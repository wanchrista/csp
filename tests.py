...
class TestStringMethods(unittest.TestCase):
    ...
    def helper_timer(self, board, prop=prop_FC, var_ord=None):
        csp, var_array = kenken_csp_model(board)
        solver = BT(csp)
        solver.quiet()
        solver.bt_search(prop, None, val_lcv)
    ...
    def helper_nary_grid(self, board):
        ...
        diff_const_count = (board[0][0]+board[0][0]) # number of all narydiff constraints
        cons = csp.get_all_cons()
        bin_count = 0 # number of binary constraints
        for c in cons:
            if len(c.get_scope()) == board[0][0]:
                bin_count += 1
        self.assertEqual(bin_count, diff_const_count, "Wrong number of n ary all different constraints for nary_ad_grid!")
    ...
    @unittest.skipUnless(TEST_TIMER, "Times up")
    def test_props_7(self):
        board = BOARDS[5]
        self.helper_timer(board, prop_FC)
